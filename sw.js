...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/app.js')){
    e.respondWith(fetch(e.request).then(async r=>{const source=await r.text();return new Response(source,{status:r.status,statusText:r.statusText,headers:r.headers})}).catch(()=>caches.match(e.request)));
    return;
  }
